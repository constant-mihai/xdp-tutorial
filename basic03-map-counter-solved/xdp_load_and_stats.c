			...
			struct stats_record *stats_prev)
{
	...
	__u64 packets;
	__u64 bytes;
	double pps, bps; /* packets per sec */

    for (uint32_t i = 0; i < XDP_ACTION_MAX; i++) {
		char *fmt_pkts = "%-12s %'11lld pkts (%'10.0f pps)"
			//" %'11lld Kbytes (%'6.0f Mbits/s)"
			" period:%f\n";
		char *fmt_bytes = "%-12s %'11lld bytes (%'10.0f bps)"
			//" %'11lld Kbytes (%'6.0f Mbits/s)"
			" period:%f\n";
		...
		printf(fmt_pkts, action, rec->total.rx_packets, pps, period);
		printf(fmt_bytes, action, rec->total.rx_bytes, bps, period);
	}
}
			  ...
			  struct stats_record *stats_rec)
{
    ...
    for (uint32_t i = 0; i < XDP_ACTION_MAX; i++) {
        map_collect(map_fd, map_type, key[i], &stats_rec->stats[i]);
    }
}
